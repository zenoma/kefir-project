...
def mean_devest_colum_generator(dataframe, output_colum_name, first_column_name, second_column_name):
    # Adding the mean and the deviation of the Storage modulus columns
    dataframe[output_colum_name] = dataframe[[first_column_name, second_column_name]].mean(axis=1)
    dataframe[output_colum_name + " devest"] = dataframe[[first_column_name, second_column_name]].std(axis=1)
    print("Created mean and devest columns in the dataframe ")


def scatter_chart_insert(dataframe):
...
def read_and_build_files():
    ...
    mean_devest_colum_generator(output, "G'", "Storage modulus_A", "Storage modulus_B")
    mean_devest_colum_generator(output, "G''", "Loss modulus_A", "Loss modulus_B")

    writer = scatter_chart_insert(output)
